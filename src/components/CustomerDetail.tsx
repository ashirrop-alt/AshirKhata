...
export function CustomerDetail({ customer, onBack }: Props) {
  ...
  return (
    <div className="h-screen flex flex-col bg-[#f8fafc] dark:bg-[#0b1224] overflow-hidden transition-colors duration-500">
      {/* Header - Glass Effect */}
      <header className="flex-none bg-white/80 dark:bg-[#0b1224]/80 backdrop-blur-md border-b border-slate-200 dark:border-white/[0.08] px-4 py-3 z-40 shadow-sm">
      ...
      <main className="flex-1 overflow-y-auto sm:overflow-hidden">
        <div className="max-w-7xl mx-auto h-full flex flex-col md:flex-row gap-4 sm:gap-6 p-4 sm:p-6">
          ...
          {/* Left Side: Balance Card with FIXED Design Line */}
          <div className="w-full md:w-80 space-y-4">
            <div className={`relative rounded-3xl p-6 sm:p-8 shadow-lg transition-all duration-300 bg-white dark:bg-[#1a2333] border border-transparent dark:border-white/[0.08] overflow-hidden`}>
              {/* The Design Line - Now visible in Dark Mode too */}
            ...
            <div className="flex flex-col gap-2 mt-4">
              <Button
                variant="outline"
                className="w-full py-5 text-sm font-semibold border-slate-200 dark:border-white/[0.1] hover:bg-slate-50 dark:hover:bg-white/[0.05] text-slate-700 dark:text-slate-300 transition-all gap-2 rounded-2xl"
              ...
              <div className="grid grid-cols-2 gap-2">
                <Button variant="outline" onClick={downloadInvoice} className="w-full py-4 text-[13px] font-semibold border-slate-200 dark:border-white/[0.08] hover:bg-slate-50 dark:hover:bg-white/[0.05] text-slate-700 dark:text-slate-300 rounded-2xl gap-1.5">
                  <History className="w-4 h-4 text-blue-500" /> PDF
                </Button>
                <Button variant="outline" onClick={shareFullHistory} className="w-full py-4 text-[13px] font-semibold border-slate-200 dark:border-white/[0.08] hover:bg-slate-50 dark:hover:bg-white/[0.05] text-slate-700 dark:text-slate-300 rounded-2xl gap-1.5">
                  History
                </Button>
              </div>
            </div>
      ...
      {/* Components like AddEntryDialog & InvoiceTemplate preserved */}
      <AddEntryDialog
        open={entryOpen}
