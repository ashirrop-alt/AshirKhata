...
export function CustomerDetail({ customer, onBack }: Props) {
  ...
  return (
    <div className="h-screen flex flex-col bg-slate-50 dark:bg-[#020617] transition-colors duration-300 overflow-hidden">
      ...
      {/* --- NAVBAR (Exact match to HomeScreen) --- */}
        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between h-12">
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col">
              ...
              {customer.phone && (
                <span className="text-[10px] font-bold text-slate-500 tracking-tight leading-none uppercase">{customer.phone}</span>
              )}
            </div>
          </div>
      ...
      <main className="flex-1 overflow-y-auto sm:overflow-hidden">
        <div className="max-w-7xl mx-auto h-full flex flex-col md:flex-row gap-4 sm:gap-6 p-4 sm:p-5">
          ...
          <div className="w-full md:w-80 space-y-4">
            <div className="relative rounded-[2rem] p-6 shadow-lg bg-white dark:bg-[#0f172a] border border-slate-200 dark:border-white/[0.05] overflow-hidden">
            ...
            <div className="grid grid-cols-2 md:grid-cols-1 gap-3">
              <Button onClick={() => { setEditingEntry(null); setEntryType("udhar"); setEntryOpen(true); }} className="h-12 sm:h-14 bg-red-600 hover:bg-red-700 text-white rounded-2xl shadow-md font-black text-xs sm:text-sm active:scale-95 transition-all">
                + Udhar Diya
              </Button>
              <Button onClick={() => { setEditingEntry(null); setEntryType("payment"); setEntryOpen(true); }} className="h-12 sm:h-14 bg-emerald-600 hover:bg-emerald-700 text-white rounded-2xl shadow-md font-black text-xs sm:text-sm active:scale-95 transition-all">
                - Paisa Mila
              </Button>
            </div>

            <div className="flex flex-col gap-2 mt-4">
               <Button variant="outline" className="w-full py-4 text-xs font-bold border-slate-200 dark:border-white/[0.05] bg-white dark:bg-[#0f172a] hover:bg-slate-50 dark:hover:bg-white/5 rounded-2xl gap-2 transition-all" onClick={sendReminder}>
                WhatsApp Reminder
              </Button>
              <div className="grid grid-cols-2 gap-2">
                <Button variant="outline" onClick={downloadInvoice} className="w-full py-3.5 text-[11px] font-bold border-slate-200 dark:border-white/[0.05] bg-white dark:bg-[#0f172a] rounded-2xl gap-2 transition-all">
                  <History className="w-3.5 h-3.5 text-blue-500" /> PDF
                </Button>
                <Button variant="outline" onClick={shareFullHistory} className="w-full py-3.5 text-[11px] font-bold border-slate-200 dark:border-white/[0.05] bg-white dark:bg-[#0f172a] rounded-2xl gap-2 transition-all">
                   History
                </Button>
              </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-3 sm:p-4 space-y-2.5">
              {[...transactions].reverse().map(tx => (
                <div key={tx.id} className="bg-slate-50 dark:bg-white/[0.03] rounded-2xl p-3.5 border border-transparent hover:border-slate-200 dark:hover:border-white/[0.05] transition-all flex items-center justify-between group">
                  <div className="flex items-center gap-3">
                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-black text-xl shadow-sm ${tx.type === "udhar" ? "bg-red-500/10 text-red-500 dark:text-red-400" : "bg-emerald-500/10 text-emerald-600 dark:text-emerald-400"}`}>
                    ...
                    <div>
                      ...
                      {tx.remarks && (
                        <p className="text-[10px] text-indigo-500 dark:text-indigo-400 font-medium italic mt-1 bg-indigo-500/10 px-2 py-0.5 rounded-lg inline-block">
                          Note: {tx.remarks}
                        </p>
                      )}
                    </div>
