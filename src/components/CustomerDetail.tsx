      ...
      <header className="flex-none bg-[#0f172a] border-b border-white/[0.05] px-4 py-3 z-40">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-4">
            <button onClick={onBack} className="p-2 -ml-2 rounded-full hover:bg-white/5 transition-all">
              <ArrowLeft className="w-6 h-6 text-slate-400" />
            </button>
            <div className="flex flex-col">
              <h1 className="text-lg sm:text-xl font-black text-slate-100 leading-none mb-1">{customer.name}</h1>
              {customer.phone && (
          ...
          <div className="flex items-center gap-2">
            {customer.phone && (
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button variant="ghost" size="icon" className="rounded-full w-10 h-10 bg-white/5 hover:bg-white/10 text-slate-400">
                    <PhoneCall className="w-5 h-5" />
                  </Button>
                </DropdownMenuTrigger>
                <DropdownMenuContent align="end" className="w-48 rounded-2xl p-2 shadow-xl border-white/10 bg-[#1e293b]">
                  <DropdownMenuItem onClick={() => makeCall('phone')} className="rounded-xl py-3 cursor-pointer gap-3 font-bold text-slate-200">
                    <Phone className="w-4 h-4 text-blue-500" /> Phone Call
                  </DropdownMenuItem>
                  <DropdownMenuItem onClick={() => makeCall('whatsapp')} className="rounded-xl py-3 cursor-pointer gap-3 font-bold text-slate-200">
            ...
            <div className="bg-white/5 p-2 rounded-lg text-slate-500">
        ...
        <div className="max-w-7xl mx-auto h-full flex flex-col md:flex-row gap-4 sm:gap-6 p-4 sm:p-6">
          ...
          <div className="w-full md:w-80 space-y-4">
            ...
            <div className={`relative rounded-3xl p-6 sm:p-8 shadow-2xl bg-[#0f172a] border border-white/[0.05] overflow-hidden`}>
              ...
              <h2 className={`text-4xl sm:text-5xl font-black tracking-tighter ${total > 0 ? "text-red-400" : "text-emerald-400"}`}>
                Rs {total < 0 ? `-${Math.abs(total).toLocaleString()}` : total.toLocaleString()}
              </h2>
            </div>

            <div className="grid grid-cols-2 md:grid-cols-1 gap-3">
              <Button onClick={() => { setEditingEntry(null); setEntryType("udhar"); setEntryOpen(true); }} className="h-14 sm:h-16 bg-red-600 hover:bg-red-700 text-white rounded-2xl shadow-lg shadow-red-500/10 font-black text-sm transition-all active:scale-95">
                + Udhar Diya
              </Button>
              <Button onClick={() => { setEditingEntry(null); setEntryType("payment"); setEntryOpen(true); }} className="h-14 sm:h-16 bg-emerald-600 hover:bg-emerald-700 text-white rounded-2xl shadow-lg shadow-emerald-500/10 font-black text-sm transition-all active:scale-95">
                - Paisa Mila
              </Button>
            </div>

            <div className="flex flex-col gap-2 mt-4">
              <Button variant="outline" className="w-full py-5 text-sm font-bold border-white/[0.05] bg-[#0f172a] hover:bg-[#1e293b] text-slate-300 gap-2 rounded-2xl" onClick={sendReminder}>
                <svg viewBox="0 0 24 24" width="18" height="18" fill="#25D366"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.414 0 .018 5.393 0 12.03c0 2.12.554 4.189 1.602 6.006L0 24l6.117-1.605a11.803 11.803 0 005.925 1.586h.005c6.635 0 12.032-5.396 12.035-12.032a11.762 11.762 0 00-3.441-8.518z" /></svg>
                WhatsApp Reminder
              </Button>
          ...
          <div className="flex-1 bg-[#0f172a] rounded-3xl border border-white/[0.05] shadow-sm overflow-hidden flex flex-col min-h-[400px]">
            <div className="px-6 py-5 border-b border-white/[0.05] flex items-center justify-between bg-white/[0.02]">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-xs font-black uppercase tracking-widest text-slate-400">Transactions ({transactions.length})</h2>
              </div>
            </div>

            <div className="flex-1 overflow-y-auto p-4 space-y-3">
              {[...transactions].reverse().map(tx => (
                <div key={tx.id} className="bg-white/[0.03] rounded-2xl p-4 border border-transparent hover:border-white/[0.05] transition-all flex items-center justify-between group">
                  <div className="flex items-center gap-4">
                    <div className={`w-12 h-12 rounded-xl flex items-center justify-center font-black text-xl ${tx.type === "udhar" ? "bg-red-500/10 text-red-400" : "bg-emerald-500/10 text-emerald-400"}`}>
                      {tx.type === "udhar" ? "+" : "-"}
                    </div>
                    <div>
                      <p className={`font-black text-xl ${tx.type === "udhar" ? "text-red-400" : "text-emerald-400"}`}>Rs {tx.amount.toLocaleString()}</p>
                      <p className="text-[10px] font-bold text-slate-500 uppercase tracking-tight">
                        {tx.type === "udhar" ? "Udhar Diya" : "Paisa Mila"} • {new Date(tx.date).toLocaleDateString("en-GB")}
                      </p>
                      {tx.remarks && (
                        <p className="text-[11px] text-indigo-400 font-medium italic mt-1 bg-indigo-500/10 px-2 py-0.5 rounded-lg inline-block">
                          Note: {tx.remarks}
                        </p>
                      )}
                    </div>
