...
export function CustomerDetail({ customer, onBack }: Props) {
  ...
  const shareFullHistory = () => {
    ...
    filteredTransactions.forEach((t) => {
      ...
      // Yahan replace lagaya hai taake 2026 chota ho kar 26 ho jaye
      const shortDate = formatDate(t.date).replace('/2026', '/26');

      message += `${shortDate}: Rs ${t.amount.toLocaleString()} ${typeText} ${typeIcon}${note}\n`;
    });
    ...
    message += `📝 *Mazeed tafseel or poora hisaab dekhne ke liye niche link par click karein:* \n`;
    message += `${shareUrl}\n\n`;
  ...
  return (
    <div className="h-screen flex flex-col bg-slate-50 dark:bg-[#020617] transition-colors duration-500 overflow-hidden">
      ...
      {/* --- HEADER --- */}
      <header className="flex-none h-16 md:h-[68px] border-b border-slate-200 dark:border-white/[0.05] bg-white dark:bg-[#0f172a] px-4 md:px-6 z-20 shadow-sm transition-all">
        <div className="max-w-7xl mx-auto h-full flex items-center justify-between">
          ...
          {/* Right Side: Enhanced Actions (Edit, Delete, Phone) - 100% HomeScreen Copy */}
          <div className="flex items-center gap-1">
            ...
            <Button
              ...
              className="w-9 h-9 bg-transparent hover:bg-amber-50 dark:hover:bg-amber-500/10 text-black/60 dark:text-white/70 hover:text-amber-500 dark:hover:text-amber-400 transition-all active:scale-95 rounded-xl"
              title="Customer badlein"
            >
              {/* Exact size 18.5px aur strokeWidth 2.2 taake look bilkul same ho */}
              <Pencil className="w-[18.5px] h-[18.5px]" strokeWidth={2.2} />
            </Button>
            ...
            <Button
              ...
              className="w-9 h-9 bg-transparent hover:bg-red-50 dark:hover:bg-red-500/10 text-black/60 dark:text-white/70 hover:text-red-500 dark:hover:text-red-400 transition-all active:scale-95 rounded-xl"
              title="Poora khata delete karein"
            >
              <Trash2 className="w-[18.5px] h-[18.5px]" strokeWidth={2.2} />
            </Button>
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button
                    variant="ghost"
                    size="icon"
                    className="w-9 h-9 bg-transparent hover:bg-blue-50 dark:hover:bg-blue-500/10 text-black/60 dark:text-white/70 hover:text-blue-600 dark:hover:text-blue-400 transition-all active:scale-95 rounded-xl border-none outline-none focus-visible:ring-0"
                  >
                    <PhoneCall className="w-[18.5px] h-[18.5px]" strokeWidth={2.2} />
                  </Button>
                </DropdownMenuTrigger>
                  ...
                  <Tooltip>
                    <TooltipTrigger asChild>
                      ...
                      >
                        ...
                        <span className="leading-none text-slate-600 dark:text-slate-300">Share</span>
                      </Button>
                    </TooltipTrigger>
    ...
    <div className="flex flex-col flex-1 px-2 py-1 min-w-0 gap-0.5">
      ...
      <div className="relative w-full flex items-center">
        ...
        <input
          ...
          onFocus={(e) => {
            if (isMobile) {
              const picker = e.target.parentElement?.querySelector('input[type="date"]') as HTMLInputElement;
              picker?.showPicker();
            }
          }}
