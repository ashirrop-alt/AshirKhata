      ...
      <main className="flex-1 overflow-hidden">
        <div className="max-w-7xl mx-auto h-full flex flex-col md:flex-row gap-4 md:gap-6 p-4 md:p-6">
          ...
          <div className="flex-1 flex flex-col min-h-0 bg-white dark:bg-[#0f172a] rounded-3xl shadow-sm border border-slate-200 dark:border-white/[0.05] overflow-hidden transition-all">
            ...
            <div className="px-4 py-4 md:px-6 md:py-4 border-b border-slate-100 dark:border-white/[0.05] bg-transparent">
    ...
    <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3" ref={dropdownRef}>
      ...
      <div className="relative">
        <button
          ...
          className="w-full sm:w-[160px] flex items-center justify-between gap-3 bg-white dark:bg-[#1e1e2d] text-slate-800 dark:text-slate-200 text-[13px] font-semibold px-4 py-2.5 rounded-xl border border-slate-200 dark:border-white/10 hover:border-indigo-500/50 shadow-sm transition-all"
        >
      ...
      {/* Date Range Picker - No More Overlapping */}
      {filterType === 'custom' && (
        <motion.div 
        ...
        >
          <div className="grid grid-cols-2 sm:flex items-center bg-slate-50 dark:bg-white/[0.03] p-1 rounded-2xl border border-slate-200 dark:border-white/10 w-full sm:w-auto">
          ...
          <button 
            ...
            className="w-full sm:w-auto flex items-center justify-center p-2.5 bg-red-50 dark:bg-red-500/10 text-red-500 rounded-xl hover:bg-red-100 transition-all active:scale-95 shrink-0"
          >
...
function DatePickerInput({ label, value, onChange }: any) {
  return (
    <div className="flex flex-col px-3 py-1.5 min-w-[120px] sm:min-w-[140px]">
      <span className="text-[9px] font-black text-indigo-500 tracking-wider mb-0.5">{label}</span>
      <input 
        type="date" 
        value={value}
        onChange={(e) => onChange(e.target.value)}
        className="bg-transparent text-[13px] font-bold outline-none text-slate-700 dark:text-slate-200 w-full [color-scheme:light] dark:[color-scheme:dark]"
      />
    </div>
  );
}