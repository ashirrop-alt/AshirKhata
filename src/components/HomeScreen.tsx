...
export function HomeScreen({ shopName, customers, isLoading, onSetShopName, onSelectCustomer, onAddCustomer }: Props) {
  ...
  return (
    <div className="h-screen flex flex-col bg-[#f9fafb] dark:bg-[#020617] text-slate-900 dark:text-slate-100 overflow-hidden transition-colors duration-500">

      {/* --- NAVBAR --- */}
      <header className="flex-none border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-[#0f172a] px-4 md:px-6 py-3 md:py-4 z-30 shadow-sm transition-all">
      ...
      <main className="flex-1 overflow-hidden">
        <div className="max-w-7xl mx-auto h-full flex flex-col md:flex-row gap-4 md:gap-6 p-4 md:p-6">
          ...
          <div className="flex-none w-full md:w-72 flex flex-col space-y-4">
            ...
            <div className="space-y-3">
              <div className="relative group">
                ...
                <Input placeholder="Customer dhunndien..." className="pl-10 h-11 rounded-xl bg-white dark:bg-[#0f172a] border-slate-200 dark:border-white/10 shadow-sm focus:ring-2 focus:ring-blue-500 transition-all dark:text-white" value={search} onChange={(e) => setSearch(e.target.value)} />
              </div>
          ...
          <div className="flex-1 flex flex-col min-h-0 bg-white dark:bg-[#0f172a] rounded-3xl shadow-sm border border-slate-300/70 dark:border-slate-800 overflow-hidden transition-all">
                ...
                {filtered.length === 0 ? (
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
                    {filtered.map(c => {
                      ...
                      return (
                       <button key={c.id} onClick={() => onSelectCustomer(c.id)} className="w-full bg-white dark:bg-white/[0.02] rounded-2xl p-4 border border-slate-200 dark:border-slate-800/60 hover:border-blue-400 dark:hover:border-blue-500/50 hover:shadow-md transition-all duration-300 group active:scale-[0.99] flex items-center justify-between">
