...
import { Plus, Store, ChevronRight, Search, LogOut, Loader2, Users, Wallet, Check } from "lucide-react";
    ...
    <div className="h-screen flex flex-col bg-slate-50 dark:bg-[#020617] text-slate-900 dark:text-slate-100 overflow-hidden transition-colors duration-300 font-sans">
      ...
      <header className="flex-none bg-white dark:bg-[#0f172a] border-b border-slate-200 dark:border-white/[0.08] px-4 py-2 z-40">
        <div className="max-w-7xl mx-auto flex items-center justify-between h-12">
          {editingShop ? (
            <form onSubmit={handleSaveShopName} className="flex items-center gap-2 bg-slate-100 dark:bg-white/5 p-1 px-3 rounded-xl border border-blue-500/30">
            ...
            <button onClick={() => setEditingShop(true)} className="flex items-center gap-3 active:scale-95 transition-all text-left group">
              <div className="bg-blue-600 p-2 rounded-lg shadow-lg shadow-blue-500/20 group-hover:rotate-3 transition-transform">
          ...
          <div className="flex items-center gap-2">
            <ModeToggle />
            <Button variant="ghost" size="icon" onClick={handleLogout} className="rounded-full w-9 h-9 bg-slate-100 dark:bg-white/5 hover:bg-slate-200 dark:hover:bg-white/10 text-slate-500 dark:text-slate-400">
      ...
      <main className="flex-1 overflow-hidden">
        <div className="max-w-7xl mx-auto h-full flex flex-col md:flex-row gap-4 sm:gap-6 p-4 sm:p-5">
          ...
          <div className="w-full md:w-80 space-y-4">
            <div className="relative rounded-[2rem] p-6 shadow-xl bg-blue-600 text-white overflow-hidden group">
            ...
            <div className="space-y-3">
              <div className="relative">
                ...
                <Input placeholder="Search customer..." className="pl-11 h-12 rounded-2xl bg-white dark:bg-[#0f172a] border-slate-200 dark:border-white/[0.08] shadow-sm focus:ring-2 focus:ring-blue-500/20 transition-all dark:text-white" value={search} onChange={(e) => setSearch(e.target.value)} />
              </div>
              <Button onClick={onAddCustomer} className="w-full h-12 sm:h-14 bg-blue-600 hover:bg-blue-700 text-white rounded-2xl shadow-lg shadow-blue-600/10 font-black text-xs sm:text-sm active:scale-[0.98] transition-all">
          ...
          <div className="flex-1 bg-white dark:bg-[#0f172a] rounded-[2rem] border border-slate-200 dark:border-white/[0.08] shadow-sm overflow-hidden flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto p-3 sm:p-4 space-y-2.5 custom-scrollbar">
              {filtered.map(c => {
                ...
                return (
                  <button key={c.id} onClick={() => onSelectCustomer(c.id)} className="w-full bg-slate-50 dark:bg-white/[0.03] rounded-2xl p-4 border border-transparent hover:border-slate-200 dark:hover:border-white/[0.1] hover:shadow-md transition-all flex items-center justify-between group text-left active:scale-[0.99]">
                    <div className="flex items-center gap-4">
                      <div className="w-11 h-11 rounded-xl bg-blue-500/10 flex items-center justify-center font-black text-blue-600 dark:text-blue-400 group-hover:bg-blue-600 group-hover:text-white transition-all shadow-sm">
                        {c.name.charAt(0).toUpperCase()}
                      </div>
                      <div>
                        <p className="font-black text-slate-900 dark:text-slate-100 text-base leading-tight group-hover:text-blue-500 transition-colors">{c.name}</p>
                        <p className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-tight mt-1">{c.transactions.length} entries</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-4">
                      <div className="text-right">
                        ...
                        <p className="text-[9px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-tight">Balance</p>
                      </div>
